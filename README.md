...
- **Windows 11（任务栏模式）**：以**透明背景**的悬浮层贴在任务栏上、融入系统风格,两行显示——第一行 IP、第二行地址,右侧带一个**刷新按钮**。**可直接拖动**到任务栏左侧或右侧,松手自动吸附到就近一侧并记住选择;靠左时自动排在**天气 / 小组件**右侧而不遮挡,靠右时停在系统托盘左侧。鼠标悬停时弹出浮层,显示含**运营商（ISP）**在内的全部详情。
...
窗口位置记录在用户数据目录的 `config.json`（浮窗模式存窗口坐标 `bounds`；Windows 11 任务栏模式存停靠侧 `taskbarSide`: `left`/`right`；macOS 存菜单栏字段 `menubarField`）：
